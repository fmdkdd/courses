...
function onLoad() {
  ...
  slots.forEach(function(slot) {
    ...
    // 3. 'click'      -> toggleSlot
    slot.addEventListener('mouseenter', highlightSelection);
    slot.addEventListener('mouseleave', unhighlightSelection);
...
function getDayElement(slot) {
	return slot.parentNode.children[0]; // indice: slide 25
}
...
function unhighlightSelection() {
  var day = getDayElement(this);
  var hour = getHourElement(this);
  day.classList.remove('selected'); // indice: opération inverse à celle du BLANK précédent
