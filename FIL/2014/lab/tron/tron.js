...
function init() {
  var screen = Screen.new(document.body, 200, 150, 4);
  input.bind(window);
...
var player = {
  ...
  checkCollision: function(screen) {
    // We crash if we are going into a non-transparent square or if we
    // are out of bounds

    var color = screen.getPixel(this.pos);
    var collides = color !== 'rgba(0,0,0,0)'
      || screen.isOutside(this.pos);

    if (collides) {
      this.pos.x = this.prevPos.x;
      this.pos.y = this.prevPos.y;
      this.dead = true;
      this.headColor = this.tailColor;
      this.draw(screen);
    }
  },
