...
define('obj', function(...pairs) {
  ...
  for (let [k, v] of pairs) {
    if (typeof k === 'string' && k.match(ident_regexp)) {
      env_write(o, k, exec_expr(v));
    } else {
      throw `Object key should be a valid identifier: '${k}'`;
    }
  }
  return o;
});
...
define('get.', function(key, obj) {
  const o = exec_expr(obj);
  const found = obj_lookup(o, key);
  if (found == null) {
    throw `Key '${key}' not in '${o}'`;
  } else {
    return env_read(found, key);
  }
});
...
define('set.', function(key, val, obj) {
  const v = exec_expr(val);
  const o = exec_expr(obj);
  if (typeof key === 'string' && key.match(ident_regexp)) {
    env_write(o, key, v);
  } else {
    throw `Object key should be a valid identifier: '${key}'`;
  }
});
...
exec_atom = function(e) {
  if (typeof e === 'number') {
    ...
    const v = e === 'this' ? lookup_this() : lookup(e);
    if (v == null) {
      throw `Unknown identifier: ${e}`;
    } else {
      return v;
...
define('nth', function(idx, vec) {
  const i = exec_expr(idx);
  const v = exec_expr(vec);
  if (i >= 0 && i < v.length) {
    return v[i];
  } else {
    throw `Invalid index into vector: ${i}`;
  }
});
...
define('push', function(elem, vec) {
  const e = exec_expr(elem);
  const v = exec_expr(vec);
  if (Array.isArray(v)) {
    v.push(e);
  } else {
    throw `Cannot push '${e}' on non-vector '${v}'`;
  }
});
...
define('pop', function(vec) {
  ...
  if (Array.isArray(v)) {
    if (v.length > 0) {
      ...
      throw `Cannot pop empty vector: '${v}'`;
    }
  } else {
    throw `Cannot pop non-vector '${v}'`;
  }
});
...
define('length', function(vec) {
  ...
  if (Array.isArray(v)) {
    ...
    throw `Non-vector '${v}' has no length`;
  }
});
