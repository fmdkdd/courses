...
function tokenize(str) {
  return str
    .replace(/[()]/g, ' $& ') // met des espaces de part et d'autre des parenthèses
    .replace(/\s+/g, ' ')     // réduit les espaces consécutifs à un seul espace
    .split(' ')               // sépare tous les tokens
    .filter(s => s.length > 0); // ignore les chaînes vides
}
...
environment.def = function(name, params, ...body) {
  environment[name] = function(...args) {
